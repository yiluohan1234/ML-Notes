...
3. 继续遍历剩余`n-k`个数。假设每一次遍历到的新的元素的值为`x`，把`x`与`kmax`比较：如果`x` < `kmax` ，用`x`替换`kmax`，并回到第二步重新找出k个元素的数组中最大元素kmax‘；如果`x >= kmax`，则继续遍历不更新数组。 每次遍历，更新或不更新数组的所用的时间为`O（k）`或`O（0）`。故整趟下来，时间复杂度为`n*O（k）=O（n*k）`。
...
3. 遍历剩余n-k个数。假设每一次遍历到的新的元素的值为`x`，把`x`与堆顶元素`kmax`比较：如果`x < kmax`，用`x`替换`kmax`，然后更新堆（用时`logk`）；否则不更新堆。 这样下来，总的时间复杂度:O（k+（n-k）*logk）=O（n*logk）。此方法得益于堆中进行查找和更新的时间复杂度均为：O(logk)（若使用解法二：在数组中找出最大元素，时间复杂度：O（k））。 解法四
